-- Generated with:
--   bun run --bun better-auth generate --config src/lib/auth/better-auth.ts --output src/db/migrations/0000_better_auth_schema.sql --yes
...
create table "auth_account" ("id" text not null primary key, "issuer" text not null, "accountId" text not null, "providerId" text not null, "userId" text not null references "auth_user" ("id") on delete cascade, "accessToken" text, "refreshToken" text, "idToken" text, "accessTokenExpiresAt" date, "refreshTokenExpiresAt" date, "scope" text, "password" text, "createdAt" date not null, "updatedAt" date not null);
